...
router.post('/', protect, asyncHandler(async (req, res) => {
  ...
  const project = await Project.create({
    ...
    status: status || 'completed', featured: featured || false, order: count,
  });
...
router.put('/:id', protect, asyncHandler(async (req, res) => {
  ...
  await project.update({
    ...
    techStack: Array.isArray(techStack) ? techStack : (techStack || '').split(',').map(t => t.trim()).filter(Boolean),
    liveUrl: liveUrl ?? project.liveUrl, githubUrl: githubUrl ?? project.githubUrl,
    imageUrl: imageUrl ?? project.imageUrl, status: status || project.status,
    featured: featured ?? project.featured, order: order ?? project.order,
  });
...
router.get('/search', asyncHandler(async (req, res) => {
  ...
  const projects = await Project.findAll({ where, limit: 20, order: [['views', 'DESC']] });
  res.json({ success: true, data: projects });
}));
