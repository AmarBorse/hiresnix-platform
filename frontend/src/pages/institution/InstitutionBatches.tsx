...
import { Plus, Pencil, Trash2, Users, X, ChevronRight, ArrowLeft } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-xl w-full max-w-lg shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h2 className="font-semibold text-gray-800">{batch ? 'Edit Batch' : 'Create Batch'}</h2>
...
const STATUS_COLORS: Record<string, string> = { Active: 'bg-green-100 text-green-700', Completed: 'bg-gray-100 text-gray-600', Upcoming: 'bg-blue-100 text-blue-700' };
...
export function InstitutionBatches() {
  ...
  const openAssign = async () => {
    try { const r = await institutionApi.getStudents({ limit: 200 }); setAllStudents(r.data); setAssignSelected([]); setAssignModal(true); }
    catch { toast.error('Failed to load students'); }
  };
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-xs text-gray-500 uppercase tracking-wide border-b border-gray-100">
              <th className="px-4 py-3">Career ID</th><th className="px-4 py-3">Name</th><th className="px-4 py-3">Email</th><th className="px-4 py-3">Department</th><th className="px-4 py-3"></th>
            </tr>
          </thead>
          <tbody>
            {studentsLoading ? <tr><td colSpan={5} className="text-center py-10 text-gray-400">Loading...</td></tr>
            : batchStudents.length === 0 ? <tr><td colSpan={5} className="text-center py-10 text-gray-400">No students in this batch yet</td></tr>
            : batchStudents.map(s => (
              <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right">
                  <button onClick={async () => { await institutionApi.removeFromBatch(viewBatch.id, s.id); toast.success('Removed'); openBatch(viewBatch); }}
  ...
  return (
    <div className="space-y-5">
      ...
      : batches.length === 0 ? <div className="bg-white rounded-xl p-12 text-center text-gray-400 border border-gray-100">No batches created yet</div>
          ...
          {batches.map(b => (
            <div key={b.id} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition">
              ...
              <div className="flex items-center gap-2 text-2xl font-bold text-indigo-600 mb-3">
                ...
                {(b as any).studentCount ?? 0} <span className="text-sm font-normal text-gray-400">students</span>
              </div>
      ...
      {modal && <BatchModal batch={modal === 'edit' ? selected : null} onClose={() => setModal(null)} onSaved={() => { setModal(null); load(); }} />}
    </div>
  );
}
