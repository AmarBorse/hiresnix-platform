...
export function StudentMockInterview() {
  ...
  const [scores, setScores]         = useState({communication:0,technical:0,confidence:0,grammar:0,problemSolving:0});
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (res.isComplete || qNumber >= 20) {
        ...
        stopCamera();
        const finalResults=[...results,{question:currentQ,answer:ans||'(No answer)',score:res.score||7,feedback:res.feedback||'',round}];
        setResults(finalResults);
        const finalAvg=finalResults.length?Math.round(finalResults.reduce((s:number,r:any)=>s+r.score,0)/finalResults.length):0;
        client.post('/mock-interview/save',{
          domain, round, difficulty, experience,
          overallScore: Math.round(finalAvg*10),
          totalQuestions: finalResults.length,
          communication: scores.communication||finalAvg,
          technical: scores.technical||finalAvg,
          confidence: scores.confidence||finalAvg,
          grammar: scores.grammar||finalAvg,
          problemSolving: scores.problemSolving||finalAvg,
          weakTopics: finalResults.filter((r:any)=>r.score<5).map((r:any)=>r.question.split(' ').slice(0,5).join(' ')+'...'),
          results: finalResults,
          duration: finalResults.length*90,
        }).catch(()=>{});
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-400">• {domain} • Q{qNumber}/15</span>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="w-full bg-gray-200 rounded-full h-1 mb-5">
        <div className="bg-indigo-500 h-1 rounded-full transition-all" style={{width:`${((qNumber-1)/15)*100}%`}}/>
      </div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      <div className={`rounded-2xl p-6 text-center text-white ${avgScore>=7?'bg-gradient-to-br from-emerald-500 to-teal-600':avgScore>=5?'bg-gradient-to-br from-amber-500 to-orange-600':'bg-gradient-to-br from-red-500 to-rose-600'}`}>
        ...
        <p className="text-sm opacity-80 mt-1">{domain} • {results.length}/15 Questions</p>
      </div>
