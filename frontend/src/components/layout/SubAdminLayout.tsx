// src/components/layout/SubAdminLayout.tsx
import React, { useState } from 'react';
import { Outlet, Link, useLocation, useNavigate } from 'react-router';
import {
  LayoutDashboard, Briefcase, FileText, GraduationCap, Building2,
  BookOpen, Award, Users, Menu, X, LogOut,
  MessageSquare, ChevronRight, ShieldHalf
} from 'lucide-react';
...
// Sub-admin can access these features only
const NAV = [
  { to: '/sub-admin/dashboard',    icon: LayoutDashboard, label: 'Dashboard'       },
  { to: '/sub-admin/iplatform',    icon: GraduationCap,   label: 'Hiresnix Intern', badge: 'NEW' },
  { to: '/sub-admin/jobs',         icon: Briefcase,       label: 'Jobs'            },
  { to: '/sub-admin/applications', icon: FileText,        label: 'Applications'    },
  { to: '/sub-admin/internships',  icon: BookOpen,        label: 'Programs'        },
  { to: '/sub-admin/students',     icon: Users,           label: 'Students'        },
  { to: '/sub-admin/companies',    icon: Building2,       label: 'Companies'       },
  { to: '/sub-admin/institutions', icon: GraduationCap,   label: 'Institutions'    },
  { to: '/sub-admin/resources',    icon: BookOpen,        label: 'Resources'       },
  { to: '/sub-admin/certificates', icon: Award,           label: 'Certificates'    },
  { to: '/sub-admin/enquiries',    icon: MessageSquare,   label: 'Enquiries'       },
];

// ❌ NOT available to sub-admin:
// - Documents, Analytics, Clients, Import Tool, Settings, Logic Builder
        ...
        <div className="mx-3 my-3 p-3 rounded-xl" style={{ background: `linear-gradient(135deg,${C.glow},rgba(255,255,255,0.03))`, border: `1px solid ${C.ring}` }}>
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: '#A78BFA' }}>Sub Admin</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="hidden md:flex h-12 items-center justify-between px-6 sticky top-0 z-10"
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-gray-400">Sub Admin Portal</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg"
            style={{ background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.2)' }}>
            <ShieldHalf size={11} style={{ color: '#A78BFA' }} />
            <span style={{ color: '#A78BFA', fontSize: '11px', fontWeight: 700 }}>Limited Access Mode</span>
          </div>
