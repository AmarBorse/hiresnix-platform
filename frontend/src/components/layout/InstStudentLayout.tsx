...
import { LayoutDashboard, Award, User, Menu, X, LogOut, GraduationCap, BookOpen, Layers, Briefcase, Sparkles } from 'lucide-react';
        ...
        <div className="mx-3 my-3 p-3 rounded-xl" style={{ background: `linear-gradient(135deg,${C.glow},rgba(255,255,255,0.03))`, border: `1px solid ${C.ring}` }}>
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] font-mono tracking-widest truncate" style={{ color: C.accent }}>{student?.careerId}</p>
            </div>
          </div>
          {student?.institutionName && (
            <p className="text-gray-600 text-[10px] mt-1.5 truncate pl-0.5">{student.institutionName}</p>
          )}
        </div>
